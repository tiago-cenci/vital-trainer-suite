...
import { SessaoExerciciosInlineBuilder } from './ExercicioRowInline';
...
export function TreinoForm({ treino, onSubmit, onCancel, isSubmitting }: TreinoFormProps) {
  ...
  const {
    ...
    defaultValues: {
      nome: '',
      aluno_id: undefined as unknown as string,
      sessoes_semanais: 5,
  ...
  const selectedPeriodizacao = periodizacoes.find(
    (p) => p?.id && String(p.id) === String(watchedValues.periodizacao_id ?? '')
  );
    ...
    if (treino && alunosProntos && perProntas) {
      ...
      const sessoesComExercicios: SessaoLocal[] = (treino.sessoes ?? []).map((s, i) => ({
        ...
        exercicios: (s.sessoes_exercicios ?? []).map((se) => ({
          ...
          series_qtd: se.series_qtd ?? undefined,
          reps_min: se.reps_min ?? undefined,
          reps_max: se.reps_max ?? undefined,
          descanso_seg: se.descanso_seg ?? undefined,
          usar_periodizacao: se.usar_periodizacao ?? false,
          exercicios: se.exercicios as Exercicio | undefined,
          series: se.series || [],
        })),
      }));
  ...
  // ===== Gerar sessões padrão ao alterar sessoes_semanais (modo criação) =====
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={(watch('sessoes_semanais') ?? 5).toString()}
        ...
        <div className="space-y-4">
          ...
          {usarPeriodizacao && (
            <div className="space-y-2">
              ...
              <Select
                value={watch('periodizacao_id') ?? undefined}
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      ...
      <SessoesBuilder
        ...
        sessoesSemanais={watch('sessoes_semanais') || 3}
      />
    </div>
  );
      ...
      <div className="space-y-6">
        {sessoes.map((sessao, idx) => (
          <SessaoExerciciosInlineBuilder
            ...
            onSessaoChange={(s) =>
              setSessoes((prev) => prev.map((x, i) => (i === idx ? s : x)))
            }
          />
        ))}
      </div>
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      <div className="min-h-[400px]">
        ...
        {currentStep === 3 && (loadingExercicios ? (
          <div className="py-8 text-center text-muted-foreground">Carregando exercícios...</div>
        ) : (
          renderStep3()
        ))}
      </div>
      ...
      <Separator />
      <div className="flex items-center justify-between">
        <Button type="button" variant="outline" onClick={currentStep === 1 ? onCancel : handlePrevious}>
          {currentStep === 1 ? (
            'Cancelar'
          ) : (
            <>
        ...
        <div className="flex gap-2">
          {currentStep < 3 ? (
            <Button type="button" onClick={handleNext} formNoValidate>
              Próximo
